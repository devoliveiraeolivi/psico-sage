...
export function SessaoTabs({
  ...
  return (
    <div className="lg:col-span-2 space-y-5">
      ...
      {/* Contexto do Paciente - sempre visível abaixo */}
      <ContextoPaciente
        pacienteResumo={pacienteResumo}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Transcrição Completa</span>
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {pacienteResumo.diagnosticos && (
          <div className="p-3 rounded-lg bg-violet-50/50 border border-violet-100">
            <div className="flex items-center gap-2 text-xs font-medium text-violet-600 mb-1.5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" />
              </svg>
              Diagnósticos
        ...
        {pacienteResumo.gatilhos && (
          <div className="p-3 rounded-lg bg-red-50/50 border border-red-100">
            <div className="flex items-center gap-2 text-xs font-medium text-red-600 mb-1.5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      {pacienteHistorico && (pacienteHistorico.marcos?.length || pacienteHistorico.insights?.length) && (
        <div className="mt-4 pt-4 border-t border-gray-100">
          <div className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">Destaques Recentes</div>
          <div className="space-y-2">
            {pacienteHistorico.marcos?.slice(0, 2).map((item, i) => (
              <div key={`marco-${i}`} className="flex items-center gap-3 text-sm">
                <span className="text-xs text-gray-400 w-14 flex-shrink-0">{formatDate(item.data)}</span>
                <span className="text-gray-700 flex-1">{item.valor}</span>
                <span className="text-xs px-1.5 py-0.5 bg-emerald-50 text-emerald-700 rounded">marco</span>
              </div>
            ))}
            {pacienteHistorico.insights?.slice(0, 2).map((item, i) => (
              <div key={`insight-${i}`} className="flex items-center gap-3 text-sm">
                <span className="text-xs text-gray-400 w-14 flex-shrink-0">{formatDate(item.data)}</span>
                <span className="text-gray-700 flex-1">{item.valor}</span>
                <span className="text-xs px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded">insight</span>
              </div>
            ))}
          </div>
        </div>
