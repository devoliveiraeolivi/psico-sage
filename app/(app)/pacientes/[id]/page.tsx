...
export default async function PacientePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: paciente } = await (supabase as any)
    ...
    .select('*')
    .eq('id', id)
    .single() as { data: Paciente | null }

  const { data: sessoesData } = await (supabase as any)
    .from('sessoes')
    .select('id, numero_sessao, data_hora, status, resumo')
    .eq('paciente_id', id)
    ...
    <div className="space-y-6">
      ...
      <div id="patient-header" className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground mt-0.5 flex items-center gap-3">
              {paciente.data_nascimento && <span>{calcularIdade(paciente.data_nascimento)} anos</span>}
        ...
        <div className="flex items-center gap-2">
          <Link href={`/pacientes/${id}/editar`} className="text-sm px-4 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors">
            Editar
          </Link>
          <Link href="/pacientes" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
            ← Voltar
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div id="patient-sidebar" className="space-y-5">
          ...
          <PatientRecurrenceConfig
            pacienteId={paciente.id}
            initialFrequencia={(paciente as any).frequencia_sessoes || null}
            initialDiaSemana={(paciente as any).dia_semana_preferido ?? null}
            initialHora={(paciente as any).hora_preferida || null}
            ...
            {sessoes.length > 0 ? (
              <div className="space-y-2">
                {sessoes.slice(0, 5).map((sessao) => (
                  <Link key={sessao.id} href={`/sessoes/${sessao.id}`} className="block p-3 rounded-lg border border-gray-100 hover:bg-gray-50 hover:border-gray-200 transition-all">
